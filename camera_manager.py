...
DEFAULT_AUTO_CAPTURE_TIME = "06:00"
...
class CameraManager:
    def __init__(self, config_path: Path) -> None:
        ...
        self._config = {
            "hflip": False,
            "vflip": False,
            "auto_capture_time": DEFAULT_AUTO_CAPTURE_TIME,
    ...
    def update_settings(self, payload: Dict[str, object]) -> Dict[str, object]:
        ...
        if "auto_capture_time" in payload:
            normalized_time = self._normalize_time_string(
                payload.get("auto_capture_time"), strict=True
            )
            updated["auto_capture_time"] = normalized_time
        if "rotation" in payload:
            try:
                rotation = int(payload.get("rotation"))
            except (TypeError, ValueError):
                raise ValueError("Rotation invalide.")
            if rotation not in (-90, 0, 90):
                raise ValueError("Rotation doit être -90, 0 ou 90 degrés.")
    ...
    def _frame_generator_usb(self) -> Iterable[bytes]:
        ...
        while True:
            ...
            if rotation:
                ...
                if ffmpeg_bin:
                    frame = self._apply_color_profile_bytes(
                        frame, rotation=rotation, ffmpeg_path=ffmpeg_bin
                    )
            yield frame
            time.sleep(0.5)
    ...
    def _capture_usb_frame_bytes(self, device: str) -> bytes:
        ...
        try:
            ...
            data = self._apply_color_profile_bytes(data)
        except subprocess.CalledProcessError as exc:
    ...
    def _apply_color_profile_bytes(self, data: bytes, rotation: int = 0, ffmpeg_path: Optional[str] = None) -> bytes:
        ...
        if not filters:
            return data
        ffmpeg = ffmpeg_path or self._find_executable("ffmpeg")
        if not ffmpeg:
            return data
        ...
        tmp_in.flush()
        tmp_in.close()
        tmp_out = Path(tmp_in.name + ".out.jpg")
        cmd = [
            ffmpeg,
            "-y",
            "-i",
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if "auto_capture_time" in data:
                    self._config["auto_capture_time"] = self._normalize_time_string(
                        data.get("auto_capture_time")
                    )
    ...
    def _normalize_time_string(self, value: Optional[str], *, strict: bool = False) -> Optional[str]:
        if value is None:
            return None
        ...
        try:
            ...
            if strict:
                raise ValueError("Heure automatique invalide (format HH:MM).")
            return None
    ...
    def _get_auto_capture_time_tuple(self) -> Optional[Tuple[int, int]]:
        normalized = self._normalize_time_string(self._config.get("auto_capture_time"))
        if not normalized:
            return None
    ...
    def _get_rotation_degrees(self) -> int:
        value = int(self._config.get("rotation", 0) or 0)
        if value not in (-90, 0, 90):
            value = 0
        return value
    ...
    def _build_rotation_filter(self) -> Optional[str]:
        rotation = self._get_rotation_degrees()
        if rotation == 90:
            return "transpose=1"
        if rotation == -90:
    ...
    def _rotate_file(self, source: Path, target: Path, rotation: int) -> None:
        ffmpeg = self._find_executable("ffmpeg")
        if not ffmpeg:
            source.replace(target)
            return
        filter_map = {90: "transpose=1", -90: "transpose=2"}
        filter_expr = filter_map.get(rotation)
        if not filter_expr:
            source.replace(target)
            return
        cmd = [
        ...
        try:
            ...
            logger.error("Rotation fichier échouée: %s", exc)
            source.replace(target)
    ...
    def _reschedule_auto_capture(self) -> None:
        ...
        if existing:
            existing.remove()
        time_tuple = self._get_auto_capture_time_tuple()
        if not time_tuple:
            return
